...
if (isset($_POST['groupid'])) {
  ...
  $sql = "SELECT*FROM groupposts WHERE groupid=? ORDER BY postdate DESC LIMIT 8 OFFSET ".$postNewCount;
    ...
    if ($resultCheck > 0) {
      while ($post = mysqli_fetch_assoc($result)) {
        ...
        if(isset($_SESSION['uid']))  {
          $postid = $post['id'];
          $uid = $_SESSION['uid'];
          $ilikeSql = "SELECT*FROM groupPostlikes WHERE postid='$postid' AND uid='$uid'";
      ...
      echo '<div class="main"><div>There are no more posts to load.</div>';
    }
  }
} else {
